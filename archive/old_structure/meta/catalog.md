...
## research_agent 사용 규칙

`research_agent`는 이 현황판을 보고 도구를 추천하되, 검증 전 프롬프트를 자동으로 가져다 쓰지 않는다.

| 상태 | research_agent 사용 기준 |
|------|--------------------------|
| `draft` | 사용 금지. 설계 참고만 가능 |
| `trialed` | 사용자가 명시적으로 원할 때만 제한적으로 사용 |
| `revised` | 후보 도구로 추천 가능. 한계와 다음 검증 항목을 함께 고지 |
| `validated` | 기본 추천 도구로 등록 가능 |
| `stable` | 표준 도구로 사용 가능 |
| `legacy` | 실행 금지. 계보 확인 및 비교 참고만 가능 |

상태를 올릴 때는 `eval/runs/`의 실제 실행 기록과 `eval/scorecard.md` 기준을 근거로 삼는다. 보기 좋은 구조나 메타데이터만으로 승격하지 않는다.
...
| res_digest_text_paper_scan | RES-002-A-scan | [학술·학위논문 1차 스캔](../packages/notebooklm/research/core/res_digest_text_paper_scan.md) / [어댑터](../packages/notebooklm/research/adapters/res_digest_text_paper_scan.notebooklm.md) / [빌드](../packages/notebooklm/research/builds/res_digest_text_paper_scan.notebooklm.md) | package | draft | 없음 | IMRaD 논문 1편 + 비정형 학술논문/학위논문 1편으로 구조 X-ray와 deep 후보 품질 확인 |
