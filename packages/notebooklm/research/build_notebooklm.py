...
# core/{base}.md + adapters/{base}.notebooklm.md  ->  builds/{base}.notebooklm.md
#
# 합본은 코어+어댑터의 "파생물"이다. 직접 고치지 말고, core/ 또는 adapters/를 고친 뒤
# 이 스크립트를 재실행해 builds/ 합본을 갱신하라. (drift 방지)
#
...
def main() -> None:
    ...
    parser.add_argument("--out", default=None,
                        help="출력 경로 (기본: <root>/builds)")
    args = parser.parse_args()

    root = Path(args.root)
    adapter_dir = root / "adapters"
    core_dir = root / "core"
    out_dir = Path(args.out) if args.out else root / "builds"
    ...
    for adapter_path in sorted(adapter_dir.glob("*.notebooklm.md")):
        ...
        print(f"build: builds/{base}.notebooklm.md  ({len(combined)} chars)")
        count += 1
